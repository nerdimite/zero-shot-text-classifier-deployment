...
export const SecondaryButton = (props) => {
  return (
    <button
      type="button"
      {...props}
      className={`inline-flex flex-none items-center px-4 py-2 border border-transparent text-base leading-6 font-medium rounded-md text-white bg-gray-600 hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-600 focus:ring-opacity-50 transition ease-in-out duration-200 ${
        props.disabled && "cursor-not-allowed"
      }`}
      disabled={props.disabled}
    >
...
export const Progress = (props) => {
  return (
    <div className="relative transition ease-in duration-500">
      <div className="overflow-hidden h-3 text-xs flex rounded-full bg-pink-200">
        <div
          style={{ width: `${props.value}%` }}
          className={`shadow-none flex flex-col text-center rounded-full whitespace-nowrap text-white justify-center bg-pink-600`}
        ></div>
      </div>
    </div>
  );
...
export const Badge = (props) => {
  return (
    <span className="py-1 px-3 font-semibold inline-block rounded-full mt-2 mb-2 text-rose-600 bg-rose-200">
      {props.label}
    </span>
  );
};
